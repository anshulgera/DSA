def notifySuccess(){
    emailext (
        ...
        subject: "Platform CI Job (Linux): ${env.JOB_NAME} - Build #: ${env.BUILD_NUMBER} - Status : ${currentBuild.currentResult}",
        body: '''${SCRIPT, template="groovy-html.template"}''',
        mimeType: 'text/html',
        from: "kiran.thuthika@jda.com",
        to: "anshul.gera@jda.com, kiran.thuthika@jda.com",
        ...
        withEnv ([
            ...
            stage('Build and Publish Docker Images') {
                techstackImage.inside ('-u root -v /var/run/docker.sock:/var/run/docker.sock -v /data/webworks_tomcat_install.properties:/opt/webworks_tomcat_install.properties' ) {
                    sh 'ant -f ' + buildWebworksDir + 'build.xml -Dbuild.type=tomcat docker'
                }
            }
